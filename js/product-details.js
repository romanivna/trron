(function () {
  ...
  const upgradeButtonComments = function () {
    ...
    xmlhttp.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        ...
        comments = document.getElementsByClassName("product-details-value--row--сomments")[0];
        comments.classList.add("product-details-value--row--" + "comments(" + quantityComments + ")");
        comments = document.getElementsByClassName("product-details-value--column--сomments")[0];
        comments.classList.add("product-details-value--column--" + "comments(" + quantityComments + ")");
        if (logInfo[0] === false) {
          return;
        } else {
          const containerRow = document.getElementsByClassName("product-details-value--row--сomments")[0];
          const containerColumn = document.getElementsByClassName("product-details-value--column--сomments")[0];
    ...
    for (key in drinkKey) {
      if (drinkKey.hasOwnProperty(key)) {
        ...
        if (Array.isArray(drinkKey[key])) {
          showAllElements(drinkKey[key], valuecontainerColumn, valuecontainerRow);
        } else {
  ...
  const showAllElements = function (value, valuecontainerColumn, valuecontainerRow) {
...
const saveComment = function (position) {
  let commentText = document.getElementsByClassName("product-details-comment-input--" + position)[0].value;
  productId = location.search.substr(1).split("_")[1];

  let time = new Date;

  time = time.getDate() + "/" + (time.getMonth() + 1) + "/" + time.getFullYear() + " " + time.getMinutes() + ":" + time.getHours();
  let coment = {};
  coment[productId] = [{
    "name": logInfo[1],
    "time": time,
