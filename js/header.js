(function () {
  ...
  const pageScrolledOrChangedWidth = function () {
    ...
    if (
      (scroll > HEIGHT_MENU_TYPE_1) &
      (width > MINIMUM_WIDTH_FOR_COMPUTER_VERSION)
    ) {
      // Attaches the navigation menu to the top of the screen
      document
        .getElementsByClassName("header-wrapper--grey")[0]
        .classList.add("header-wrapper--grey--fixed");
      // Changes the position
      document
        .getElementsByClassName("header-nav-catalog")[0]
        .classList.add("header-nav-catalog--static");
      // Fix marggin
      document
        .getElementsByClassName("header-nav-catalog-list-list")[0]
        .classList.add("header-nav-catalog-list-list--scroll");
      // Displays the button navigation (search, user, shopping cart) in the navigation bar
      document
        .getElementsByClassName("header-nav-scroll")[0]
        .classList.add("header-nav-scroll--show");
      // Кeduces the menu button (takes the name of the "menu")
      document
        .getElementsByClassName("header-nav-menu-name")[0]
        .classList.add("header-nav-menu-name--none");
      document
        .getElementsByClassName("header-wrapper")[0]
        .classList.add("header-wrapper--fixedMenu");
    } else if (
      (scroll > HEIGHT_MENU_TYPE_2) &
      (width < MINIMUM_WIDTH_FOR_COMPUTER_VERSION)
    ) {
      ...
      document
        ...
        .classList.add("container-stop-link--index");
      document
        .getElementsByClassName("header-nav-menu-name")[0]
        .classList.remove("header-nav-menu-name--none");
      document
        .getElementsByClassName("header-nav-scroll")[0]
        .classList.remove("header-nav-scroll--show");
      document
        .getElementsByClassName("header-wrapper")[0]
        .classList.add("header-wrapper--fixedMenu");
    } else {
      ...
      document
        .getElementsByClassName("header-wrapper--grey")[0]
        .classList.remove("header-wrapper--grey--fixed");
      document
        .getElementsByClassName("header-nav-menu-name")[0]
        .classList.remove("header-nav-menu-name--none");
      document
      ...
      document
        .getElementsByClassName("header-nav-catalog")[0]
        .classList.remove("header-nav-catalog--static");
      document
        .getElementsByClassName("header-nav-catalog-list-list")[0]
        .classList.remove("header-nav-catalog-list-list--scroll");
      document
        .getElementsByClassName("header-wrapper")[0]
        .classList.remove("header-wrapper--fixedMenu");
    }
  ...
  document
    ...
    .addEventListener("click", function () {
      ...
      document
        .getElementsByClassName("header-top-list")[0]
        .classList.remove("header-top-list--click");
      document
        .getElementsByClassName("header-mid-search-list")[0]
